...
#output: html_notebook # Notebook format
---
...
library(pacman)
p_load(scales,dplyr,readr,psych,tidyr,knitr,userfriendlyscience,afex,flextable,officer,emmeans,ggplot2,ggpubr,cowplot,stringr,plyr)
...
```
## Reliability
```{r ReliabilityCalculation, echo=FALSE, warning=FALSE}
################################################################
...
# runction. S needs to be run on properly reverse coded data. This data is organised in the seperate
# calc.rel function.
...
```{r prelim_tests, echo=FALSE, message=FALSE, warning=FALSE}
...
theme_set(
  theme_pubr()  #axes
  #scale_x_continuous(breaks=seq(min,max),limits=c(min, max))+
  #coord_fixed(ratio=1)+ #ration of y-axis to x-axis
  #theme_minimal(base_size = 15)+ # no axes
  #theme_pubr(base_size = 15)+ #axes
...
# covariate plot function
  ...
  if(anova.out["mediatype","Pr(>F)"]>.05) {
    ...
    x<-as.data.frame(unlist(stby(wider[dv],wider$mediatype,descr, stats=c("Mean","sd"),transpose=TRUE)))
  ...
  #generate plot (always show this plot)
  ifelse(dv=="TIME",
  ...
  ggsave(filename=paste0(dv,".png"), device = "png", 
       path="output/",
         width = 19, units = "cm", 
       dpi = 600)
...
plot(ggplot(heat_long, aes(x = measure, dv)) +
  ...
  ggsave(filename="heatmap.png", device = "png", 
       path="output/",
         width = 25, units = "cm", 
       dpi = 600)
```